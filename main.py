...
if __name__ == '__main__':
    ...
    if(F1_train):
        ...
        # Training of first forward model
        for F1_epoch_iter in range(F1_train_epochs):
            ...
            print("| {}:{}/{} | fwdEpochTrainLoss: {:.2e} | fwdEpochTestLoss: {:.2e}".format("fwd_1",F1_epoch_iter,F1_train_epochs,F1_train_loss,F1_test_loss))
    ...
    if(F2_train):
        ...
        # Training of second forward model
        for F1_epoch_iter in range(F2_train_epochs):
            ...
            print("| {}:{}/{} | fwdEpochTrainLoss: {:.2e} | fwdEpochTestLoss: {:.2e}".format("fwd_2",F1_epoch_iter,F2_train_epochs, F2_train_loss, F2_test_loss))
    ...
    print('F1 test reconstruction C R2:',F2ComponentR2Y,'\n')
    invComponentR2Y = computeR2(C_test_pred_pred, C_test)
    print('Inv test reconstruction C R2:',invComponentR2Y,'\n')
    ...
    C_ort_test = C_ort_scaling.unnormalize(C_ort_test)
    C_ort_test_pred = C_ort_scaling.unnormalize(C_ort_test_pred)
    ...
    temp, topology_test = torch.split(F1_features_test, [4,27], dim=1)
    topology = decodeOneHot(topology_test,'shift')
    F1_features_test = torch.cat((temp,topology),dim=1)

    temp_pred, topology_test_pred = torch.split(F1_features_test_pred, [4,27], dim=1)
    topology_pred = decodeOneHot(topology_test_pred,'shift')
    F1_features_test_pred = torch.cat((temp_pred,topology_pred),dim=1)

    angle_axis_pred = rotation_6d_to_angleaxis(R1_test_pred)
    angle_axis_pred2 = rotation_6d_to_angleaxis(R2_test_pred)

    F1_features_test = F1_features_scaling.unnormalize(F1_features_test)
    V_test = V_scaling.unnormalize(V_test)
    F1_features_test_pred = F1_features_scaling.unnormalize(F1_features_test_pred)
    V_test_pred = V_scaling.unnormalize(V_test_pred)

    F1_features_test = torch.cat((F1_features_test,R1_test,R2_test,V_test),dim=1)
    full_pred = torch.cat((F1_features_test_pred,angle_axis_pred,angle_axis_pred2,V_test_pred),dim=1)

    F1_features_test = F1_features_test.cpu()
    full_pred = full_pred.cpu()
    C_test = C_test.cpu()
    C_test_pred = C_test_pred.cpu()
    C_test_pred_pred = C_test_pred_pred.cpu()
    
    exportTensor("TestReg/F1_features_test",F1_features_test,all_names)
